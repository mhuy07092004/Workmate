...
function JobCard({ job }) {
  // Support both the new schema (employmentType + workArrangement) and the
  // legacy type field so existing mock data keeps working unchanged.
  const employmentLabel = job.employmentType || job.type
  const arrangementLabel = job.workArrangement && job.workArrangement !== job.employmentType
    ? job.workArrangement
    : null
    ...
    <article className="bg-white rounded-[14px] p-6 shadow-[0_2px_12px_rgba(15,23,42,0.07)] hover:shadow-[0_4px_20px_rgba(15,23,42,0.12)] transition-shadow cursor-pointer">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Tags row: employment type, work arrangement, location */}
      ...
      {/* Footer row: posted time + optional salary */}
